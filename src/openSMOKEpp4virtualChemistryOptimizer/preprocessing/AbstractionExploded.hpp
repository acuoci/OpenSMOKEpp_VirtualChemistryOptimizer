...
namespace OpenSMOKE
{
	...
	{
		unsigned int n_existing = 0;
		if (std::find(reactant_species.begin(), reactant_species.end(), species_Rp_) != reactant_species.end())
		{
			if (std::find(product_species.begin(), product_species.end(), species_RpH_) != product_species.end())
			{
				for (unsigned int i = 0; i < n_; i++)
				{
					if (std::find(reactant_species.begin(), reactant_species.end(), species_RH_[i]) != reactant_species.end())
					{
						if (std::find(product_species.begin(), product_species.end(), species_R_[i]) != product_species.end())
						{
							is_existing_[i] = true;
							n_existing++;
						}
