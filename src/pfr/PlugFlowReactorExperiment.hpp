...
namespace OpenSMOKE
{
		...
		OpenSMOKE::VirtualChemistry*			virtual_chemistry)
	{
		...
		OpenSMOKE::PlugFlowReactor_Type type;
		{
			std::string value;
			if (dictionaries(main_dictionary_name_).CheckOption("@Type") == true)
			{
				dictionaries(main_dictionary_name_).ReadString("@Type", value);
				if (value == "Isothermal")	type = OpenSMOKE::PLUGFLOW_REACTOR_ISOTHERMAL;
				else if (value == "NonIsothermal")	type = OpenSMOKE::PLUGFLOW_REACTOR_NONISOTHERMAL;
		...
		OpenSMOKE::PlugFlowReactor_Profile*	profile;
		{
			...
			{
				if (type == OpenSMOKE::PLUGFLOW_REACTOR_NONISOTHERMAL)
		...
		// Ignition Delay Times
		{
			...
			{
				if (type == OpenSMOKE::PLUGFLOW_REACTOR_ISOTHERMAL)
		...
		// Optimization
		{
			optimization_ = new OpenSMOKE::OptimizationRules_PlugFlowReactor;
		...
		if (type == OpenSMOKE::PLUGFLOW_REACTOR_ISOTHERMAL)
		{
			plugflow_ = new OpenSMOKE::PlugFlowReactor_Isothermal(*thermodynamicsMapXML, *kineticsMapXML,
				*ode_parameters_, *plugflow_options_, *onTheFlyROPA_, *on_the_fly_post_processing_, *idt, *polimi_soot_, *virtual_chemistry, time_independent_variable, constant_pressure,
				velocity, T, P_Pa, omega);

			if (temperature_profile == true)
				plugflow_->SetTemperatureProfile(*profile);
		}
	}
	...
	void PlugFlowReactorExperiment::Solve(const bool verbose)
	{
		double T;
		double P;
		OpenSMOKE::OpenSMOKEVectorDouble omega(thermodynamicsMapXML_->NumberOfSpecies());

		const unsigned int index = thermodynamicsMapXML_->IndexOfSpecies(optimization_->species_name());

		norm2_abs_error_ = 0.;
		norm2_rel_error_ = 0.;
		for (unsigned int i = 0; i < optimization_->np(); i++)
		{
			plugflow_->Solve(optimization_->x(i));
			plugflow_->GetFinalStatus(T, P, omega);
			y_(i) = omega[index];

			const double abs_error = (optimization_->y(i) - y_(i));
			const double rel_error = (optimization_->y(i) - y_(i))/ optimization_->y(i);
			norm2_abs_error_ += abs_error * abs_error;
			norm2_rel_error_ += rel_error * rel_error;
		}

		norm2_abs_error_ /= static_cast<double>(optimization_->np());
		norm2_rel_error_ /= static_cast<double>(optimization_->np());

		// TODO
		norm2_abs_error_ /= optimization_->max_y();
		//norm2_rel_error_ /= static_cast<double>(optimization_->np());
